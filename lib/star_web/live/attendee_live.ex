defmodule StarWeb.AttendeeLive do
  ...
  def handle_params(
    ...
    enrollment =
      if enrollment.user.identifier == user_identifier do
        ...
        %{status: "ERROR", course_session: %{course: %{title: "", cover_url: "https://res.cloudinary.com/carlogilmar/image/upload/v1601588436/visual_partner/IMG_0154_osxkwq.png"}}}
      end
  ...
  def handle_event("attendee_course", %{}, socket) do
    {:ok, enrollment} = EnrollmentOperator.update(socket.assigns.enrollment.id, %{"status" => "READY"})
    socket =
      socket
      |> assign(:enrollment, enrollment)
    {:noreply, socket}
